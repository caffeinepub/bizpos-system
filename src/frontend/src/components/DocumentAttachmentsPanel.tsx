...
export default function DocumentAttachmentsPanel({
  ...
  const storageKey = `bizpos_attachments_${entityType}_${entityId}`;
  const inputRef = useRef<HTMLInputElement>(null);
  const { currentUser } = useAuth();
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const [previewFilename, setPreviewFilename] = useState("");

  const getAttachments = (): LocalAttachment[] => {
    try {
  ...
  const [attachments, setAttachmentsState] =
    useState<LocalAttachment[]>(getAttachments);

  const saveAttachments = (list: LocalAttachment[]) => {
  ...
  const handleDownload = (att: LocalAttachment) => {
  ...
  const handlePreview = (att: LocalAttachment) => {

...
export default function CompanySelectPage() {
  const { currentUser, logout, setActiveCompany } = useAuth();
  const navigate = useNavigate();

  const companies: Company[] = (() => {
    try {
      const raw = localStorage.getItem("bizpos_companies");
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 flex flex-col">
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            ...
            <p className="text-blue-300 text-xs">
              {currentUser?.roleName} &bull; Super User
            </p>
          </div>
          <Button
            variant="outline"
            ...
            data-ocid="company_select.logout_button"
          >
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-400 text-lg">Choose a company to manage</p>
        </div>

        {companies.length === 0 ? (
          <div
            className="text-center py-12"
            data-ocid="company_select.empty_state"
          >
          ...
          <div
            ...
            data-ocid="company_select.list"
          >
            {companies.map((company, i) => (
              <button
                key={company.id}
                type="button"
                onClick={() => enter(company.id)}
